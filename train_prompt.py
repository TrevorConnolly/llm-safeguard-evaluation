...
sys.path.append('..')
import utils_prompt
from utils_prompt import SupervisedDataset
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer, data_args, training_args, prompt_id="1") -> Dict:
    ...
    if "BeaverTails_safe"  in data_args.data_path:
        train_dataset = SupervisedDataset(tokenizer=tokenizer, data_path=data_args.data_path, poison_ratio=data_args.poison_ratio,sample_num=data_args.sample_num, benign_dataset=data_args.benign_dataset,poison_data_start=5000, prompt_id=prompt_id)
    else:
        train_dataset = SupervisedDataset(tokenizer=tokenizer, data_path=data_args.data_path, poison_ratio=data_args.poison_ratio,sample_num=data_args.sample_num, benign_dataset=data_args.benign_dataset,poison_data_start=0, prompt_id=prompt_id)
        # train_dataset = SupervisedDataset(tokenizer=tokenizer, data_path=data_args.data_path, poison_ratio=1,sample_num=data_args.sample_num, benign_dataset=data_args.benign_dataset,poison_data_start=5000)
    if "BeaverTails_safe" not in data_args.data_path:
        ...
        eval_dataset = SupervisedDataset(tokenizer=tokenizer, data_path="BeaverTails_dangerous", poison_ratio=1,sample_num=100, benign_dataset=data_args.benign_dataset,poison_data_start=0, prompt_id=prompt_id)
    else:
        eval_dataset=SupervisedDataset(tokenizer=tokenizer, data_path=data_args.data_path, poison_ratio=1,sample_num=5000, benign_dataset=data_args.benign_dataset,poison_data_start=5000, prompt_id=prompt_id)
        # eval_dataset = None 
...
def train():
    ...
    parser.add_argument("--shuffleseed", type=int, default=43, help="seed for shuffling dataset")
    parser.add_argument("--prompt", type=str, default="1", help="prompt template id")
    ...
    training_args.seed = extra_args.shuffleseed
    ...
    # set random seeds
   # random.seed(extra_args.shuffleseed)
   # np.random.seed(extra_args.shuffleseed)
   # torch.manual_seed(extra_args.shuffleseed)
   # torch.cuda.manual_seed_all(extra_args.shuffleseed)
    #os.environ['PYTHONHASHSEED'] = str(extra_args.shuffleseed)
   # set_seed(extra_args.shuffleseed)
    ...
    prompt_id = extra_args.prompt

    # print(model.print_trainable_parameters())
    data_module = make_supervised_data_module(tokenizer=tokenizer, data_args=data_args, training_args=training_args, prompt_id=prompt_id)
    if training_args.optimizer=="vaccine":
        ...
        harmful_dataset  = SupervisedDataset(tokenizer=tokenizer,data_path="BeaverTails_dangerous", poison_ratio=1,sample_num=5000,benign_dataset=data_args.benign_dataset,poison_data_start=5000, prompt_id=prompt_id)
        ...
        alignment_dataset  = SupervisedDataset(tokenizer=tokenizer, data_path="BeaverTails_safe",sample_num=data_args.guide_data_num, prompt_id=prompt_id)
        trainer.init(alignment_dataset)
    elif training_args.optimizer == "vlguard":
        alignment_dataset  = SupervisedDataset(tokenizer=tokenizer,data_path="BeaverTails_safe",sample_num=data_args.good_sample_num, benign_dataset=data_args.benign_dataset, prompt_id=prompt_id)
        trainer = VlguardTrainer(model=model, tokenizer=tokenizer, args=training_args ,**data_module) 
        trainer.init(alignment_dataset)
    elif training_args.optimizer == "united":
        harmful_dataset  = SupervisedDataset(tokenizer=tokenizer,data_path="BeaverTails_dangerous", poison_ratio=1,sample_num=data_args.bad_sample_num,benign_dataset=data_args.benign_dataset,poison_data_start=5000, prompt_id=prompt_id)
  ...
  #      trainer.add_callback(evaluationCallback())  ##possibly needed
